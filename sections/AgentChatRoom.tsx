...
import { Ellipsis, Loader, Loader2 } from 'lucide-react';
...
const AgentChatRoom = ({id}: IdProps) => {
...
useEffect(() => {
  axiosClient.get(`/agent/contacts/${id}`)
    .then((response) => {
...
//Automatically fetch chats onmount
useEffect(() => {
  let buyerId = localStorage.getItem("buyerId");
  if (buyerId != undefined) {
      axiosClient.get(`/agent-to-buyer-chats/${id}/${buyerId}`)
      .then(response => {
...
//Fetch chats when client is clicked
const startChat = (item: any) => {
      setStartLoading(true)
      axiosClient.get(`/start-agent-to-buyer-chats/${id}/${item.buyer_id}`)
      .then(response => {
          setStartLoading(false)
          setChats(response.data)
          setBuyerId(item.buyer_id)
          localStorage.setItem('buyerId', item.buyer_id)
          // console.log(response.data)
        })
        .catch(error => {
          console.log(error)
...
const onSubmitChat = () => {
    ...
    setIsLoading(true)
    axiosClient.post(`/agent-chat`, payLoad)
     .then(response => {
        setIsLoading(false)
    ...
    <div className="flex h-[550px] max-sm:h-h-[600px] overflow-hidden text-sm">
              ...
              <div className="flex flex-col justify-start items-start gap-3">
                {
                  ...
                  contacts.map((item: {
                    buyer_chats: any;
                    buyer_id: any; buyer_name: string | any; }, i: React.Key | null | undefined) => (
                    <div 
                      key={i} 
                      onClick={() => startChat(item)}
                      className={`flex justify-center items-center space-x-1 hover:bg-gray-100 p-1 rounded ${item.buyer_id == buyerId ? "bg-main-100/60": ""}`}
                    >
                      <div className="w-7 h-7 bg-gray-300 rounded-full mr-3">
                        ...
                        {/* <Image 
                            src={`${process.env.NEXT_PUBLIC_BACKEND_DEVELOPMENT_API}/storage/${item.sign.signature_path}`}
                            width={60}
                            height={50}
                            alt=''
                      ...
                      <div className='flex flex-col'>
                          <div className='flex items-center space-x-1'>
                            <h2 className="text-md font-semibold">{item.buyer_name}</h2>
                            {
                              item.buyer_chats.length > 0 
                              &&
                              item.buyer_chats[0].is_read == "No"
                              &&
                          ...
                          <i></i>
                            {
                              ...
                              <p className="text-gray-900 text-xs max-sm:hidden italic truncate w-[60%] font-bold">
                                {item.buyer_chats.length > 0 ? item.buyer_chats[0].message : "No message yet"}
                              </p>
                              :
        ...
        <div className="flex flex-col justify-between w-full">
            <div className=''>
              ...
              {chats ? <h1 className="text-2xl max-sm:text-lg font-semibold"> {chats && chats[0]?.buyer?.name}</h1> : "No Messages"}
              </header>
                  ...
                  <div className="h-[400px] overflow-y-auto p-4 pb-36">
                    {
                        chats 
                        ?
                        chats.map((chat: { sender: string; agent_id: any; message: string | null | undefined; agent: { photoUrl: any; }; }, i: any) => (
                          ...
                          <div key={i}>
                            {
                              ...
                              <div className="flex mb-4 cursor-pointer">
                                <div className={`w-9 h-9 rounded-full flex items-center justify-center mr-2 ${chat.agent_id === id ? 'bg-indigo-500 text-white' : ""}`}>
                            ...
                            
                            {
                              ...
                              <div className="flex justify-end mb-4 cursor-pointer">
                                ...
                                <div className="w-9 h-9 rounded-full flex items-center justify-center ml-2">
                                  <img src={`${chat.agent.photoUrl}`} alt="My Avatar" className="w-8 h-8 rounded-full"/>
                                </div>
                              </div>
                            }
                        ...
                        <div className='flex justify-center items-center h-full'>Click a client to begin the chat</div>
                      }
                  </div>
              }
            </div>
            ...
            <footer className="bg-white border-t border-gray-300 p-2 w-full">
                ...
                <div className="flex items-center py-2 bg-gray-50 rounded-lg dark:bg-gray-700">
                    ...
                    <textarea
                     ...
                     className="block p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your message..."
                    ></textarea>
                    <button
                     type="submit"
