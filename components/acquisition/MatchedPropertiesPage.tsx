...
import { savePropertyAcquisition } from '@/lib/actions/acquisition.actions';
...
// --- Main Page Component ---
const MatchedPropertiesPage = ({propertyData, client} : {propertyData: any, client: any}) => {
    const router = useRouter()
    const [savedProperties, setSavedProperties] = useState<number[]|any>([]);
    ...
    const handleSave = (id: number, title: string | any): void => {
        if (client.length > 0) {
            if (!savedProperties.includes(id)) {
                if (savedProperties.length < 1) {
                    setSavedProperties([...savedProperties, `${id};${title}`]);
                    alert(`Property ${id} saved!`);
                    typeof window !== "undefined" && window.localStorage.setItem("saved_property_acquisition", JSON.stringify({"property_id": id, "property_title": title}))
                    setIsActionTaken(true);
                }else{
                    toast.warning("For now we only allow one property to be saved at a time.");
                }
            } else {
                alert(`Property ${id} is already saved.`);
            }
        }else{
            alert("You need to log in as a client first...");
            typeof window !== "undefined" && window.localStorage.setItem("callerpage", "/property/acquisition/")
    ...
    const handleCompareClick = (): void => {
        const propertyTitles = comparedProperties.map(id => {
            const property = propertyData.find((p: any) => p.id === id);
            return property ? property.title : 'Unknown Property';
        }).join(', ');
        alert(`Comparing properties: ${propertyTitles}`);
    };

    // console.log(client.whoId.split(";")[1]);

    const handleProceedClick = async () => {
        try {
            const payLoad = {
                client_id: client.id,
                property_id: savedProperties[0].split(";")[0],
                title: savedProperties[0].split(";")[1]
            }
            const response = await savePropertyAcquisition(payLoad)
            // console.log(response);
            const resp = JSON.parse(response);
            if (resp.status === 401) {
                return toast.warning(resp.msg);
            }else{
                toast.warning(resp.msg)
            }

            toast.success("Your property was successfully saved. Property verification will not proceed on this")
        ...
        <div className="bg-gray-100 min-h-screen px-6 font-sans pt-28 pb-10">
            ...
            <main className=" mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {propertyData.slice(0,3).map((property: any) => (
