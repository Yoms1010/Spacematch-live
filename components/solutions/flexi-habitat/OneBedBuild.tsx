...
declare interface OneBedPrefabProps {
...
function OneBedBuild({prefabEquipments, homeData, setModalOpen, setOpenModal}: OneBedPrefabProps) {

        const [oneBedTotalCost, setOneBedTotalCost] = useState()
        ...
        function handleEquipment(): void {
            ...
            setOneBedTotalCost(total + homeData?.cost)
        }
            ...
            const oneBed = [
                {
                    ...
                    total: oneBedTotalCost
                }
            ]
    
            localStorage.setItem("oneBed", JSON.stringify(oneBed))
            setOpenModal(false)
            setModalOpen(true)
        }
    ...
    <div className='flex flex-col gap-3 text-sm'>
        ...
        <div className='flex justify-center items-center border p-2 w-full'>
            <span className='font-semibold text-xl'>
                <CurrencyFormat value={oneBedTotalCost || homeData?.cost} displayType={'text'} thousandSeparator={true} prefix={"₦"}/>
            </span>
        </div>
