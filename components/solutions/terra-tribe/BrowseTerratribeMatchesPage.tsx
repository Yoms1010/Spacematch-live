...
const MatchCard: React.FC<MatchCardProps> = ({ match, onConnectClick }) => (
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden group hover:shadow-xl hover:scale-[1.01] transition duration-300">
        <img 
            src={"https://placehold.co/600x400/C89F5B/FFFFFF?text="+match.partner_type} 
            alt={match.name} 
        ...
        <div className="p-5">
            <div className="flex justify-between items-center mb-2">
                ...
                <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2.5 py-0.5 rounded-full shadow-inner">
                    {Math.floor((match.percentage/12)*100)}% Match
                </span>
            </div>
            <p className="text-gray-600 mt-1 mb-3 text-sm line-clamp-2">{match.description}</p>
            <div className="text-sm text-gray-500 space-y-1">
                <p><strong>Budget:</strong> ₦{Number(match.budget).toLocaleString()}</p>
                <p><strong>Location:</strong> {match.lga}, {match.state}</p>
                <p><strong>Share:</strong> {match.contribution}</p>
            </div>
...
// --- Main BrowseTerratribeMatchesPage Component ---
const BrowseTerratribeMatchesPage = ({matches}: {matches : any}) => {
    ...
    const filteredMatches = useMemo(() => {
        ...
        return matches.filter((match: any) => {
            // Check 1: Budget
            const budgetMatch = match.budget <= maxBudget;

            // Check 2: Share
            const shareMatch = !filterShare || filterShare === 'All Shares' || match.contribution === filterShare;

            // Check 3 & 4: Location (State and LGA)
            // Note: Location filtering checks if the location string includes the filter value.
            const stateMatch = !filterState || filterState === 'All States' || match.state.includes(filterState);
            const lgaMatch = !filterLga || filterLga === 'All LGAs' || match.lga.includes(filterLga);
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredMatches.length > 0 ? (
                                filteredMatches.map((match: any, i:Key) => (
                                    <MatchCard 
                                        key={i} 
                                        match={match} 
...
export default BrowseTerratribeMatchesPage;
