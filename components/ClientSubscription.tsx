...
declare interface ClientSubscriptionProps {
    user: {whoId: string}|any, 
    subscriptionCard: any
}

function ClientSubscription({user, subscriptionCard}: ClientSubscriptionProps) {
  ...
  return (
    <div className="grid lg:grid-cols-3 px-8 gap-10 text-zinc-800 mt-10 mx-auto">
        {
            subscriptionCard && subscriptionCard.map((item: any, i: any) => (
                <div
                    key={i}
                    className={`flex flex-col items-center ${item.title == "Free" ? "bg-slate-200" : item.title == "Basic" ? "bg-gradient-to-br from-green-100 via-main-100 to-purple-100 text-white" : item.title == "Premium" ? "bg-gradient-to-br from-main-100 via-black-1 to-gray-600 text-white" : "bg-gradient-to-br from-red-300 via-black-1 to-main-100"} p-8 rounded-lg shadow-lg relative border-8 ${item.title == "Elite" && "border-main-100"} max-w-sm ${item.text_color}`}>
                    {
                        item.title == "Premium"
                        &&
                    ...
                    <div>
                        ...
                        <p className="opacity-60 text-center">{item.description}
                        ...
                        <div className="flex gap-4 justify-center">
                            <div className="flex flex-col items-center my-8">
                                <p className="font-extrabold text-4xl">₦{item.cost_naira}

                                </p>
                                <p className="text-sm opacity-60">Per Month
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex justify-center mt-8">
                            {
                                user?.isSubscribed === "Yes" && user?.client_sc_id == item.id
                                ?
                                <div className='w-full p-2 border text-center text-white font-semibold'>Subscribed</div>
                                :
                                item.title === "Free"
                                ?
                                <button 
                                    onClick={handleFreePlanSubscription}
                                    className="px-4 py-2 border-main-100 border-4 hover:bg-violet-100 rounded-xl">
                                    {
                                        isLoading
                                        ?
