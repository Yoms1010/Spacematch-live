        ...
        try {
            ...
            if (response.status === 200) {
                if (response.data.status === 201) {
                    ...
                    setIsPlaying(true)
                    const resp = JSON.parse(response.data.resData);
                    typeof window !== "undefined" && window.localStorage.setItem("otp", resp?.otp)
                    setUserId(resp?.userId)
                    setClientId(resp?.buyerId)
        ...
        <div>
            {
                sendOtp
                    ?
                    <ClientForgotPassword userId={userId} buyerId={clientId} buyerEmail={form.email} duration={duration} setDuration={setDuration} startCountdown={startCountdown} setStartCountdown={setStartCountdown} isPlaying={isPlaying} setIsPlaying={setIsPlaying} />
                    :
                    <form onSubmit={handleSubmit} className="p-6 bg-white rounded-xl shadow-lg w-full max-w-3xl mt-20">
                        ...
                        <button
                            ...
                            className={`flex justify-center items-center w-full mt-6 py-3 px-4 text-white font-semibold rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-main-100 focus:ring-offset-2 transition-all duration-200 ${isClientAgreed ? 'bg-main-100 hover:bg-main-100' : 'bg-gray-400 cursor-not-allowed'}`}
                        >
                            {
                                isLoading
