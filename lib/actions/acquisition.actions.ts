...
export const savePropertyAcquisition = async (payload: any): Promise<any> => {
  try {
      const apiKey = await getToken();
      const apiEndpoint = `${process.env.BACKEND_DEVELOPMENT_API}/save-property-acquisition/`;
  
      const {client_id, property_id, title,} = payload;
